// Wbudowane 2024, Wojciech Kieloch, zad.1 l10
...
void LCD_Initialize(void)
{
  ...
  LCD_WriteCommand(HD44780_DISPLAY_ONOFF | HD44780_DISPLAY_ON | HD44780_CURSOR_ON | HD44780_CURSOR_NOBLINK); // włącz LCD, bez kursora i mrugania
}
...
void moveRowBuffer()
{
  ...
  {
    displayBuffer[i] = displayBuffer[i + 16];
    displayBuffer[i + 16] = ' ';
  }
  if (cursor < 16)
    cursor = 0;
...
{
  LCD_GoTo(0, 0);
  printf("%.16s", displayBuffer);
  LCD_GoTo(0, 1);
  printf("%.16s", displayBuffer + 16);
  LCD_GoTo(0, 0);
}
...
int main()
{
  ...
  while (1)
  {
    ...
    if (in == '\n')
      ...
      appendCharToBuffer(in);
    printBuffer();

    const uint8_t column = cursor < 16 ? 0 : 1;
    const uint8_t row = cursor < 16 ? cursor : cursor - 16;
    LCD_GoTo(row, column);
  }
}
